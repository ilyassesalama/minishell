...
/*   Updated: 2023/07/24 15:05:49 by isalama          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		if (input[i] == '\'' || input[i] == '\"')
		{
			...
			if (input[i] != quote)
				return (g_global.exit_status = 1,
					out_error(ERROR_MSG_QUOTE), false);
			else
				i++;
		}
		else
	...
	if (tokens -> type == PIPE)
	{
		...
		if (tokens -> next == NULL || tokens -> next -> type == PIPE)
			return (g_global.exit_status = 258,
				out_error(ERROR_MSG_SYNTAX), true);
	}
...
bool	syntax_error(t_token *tokens)
{
	if (tokens && tokens -> type == PIPE)
		return (g_global.exit_status = 258, out_error(ERROR_MSG_SYNTAX), false);
	while (tokens)
	{
