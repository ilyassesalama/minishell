...
/*   Updated: 2023/05/20 05:42:42 by tajjid           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_command	*command_creator(t_token *tokens)
{
	...
	while (tmp_tokens)
	{
		...
		else if (tmp_tokens->type == APPEND)
		{
			...
			else if (tmp_tokens->next->type == SINGLE_QUOTE)
			{
				output = fd_opener(tmp_tokens->next->content, 1);
				tmp_tokens = tmp_tokens -> next;
			}
			else if (tmp_tokens->next->type == SPACE && tmp_tokens->next->next->type == SINGLE_QUOTE)
			{
				output = fd_opener(tmp_tokens->next->next->content, 1);
				tmp_tokens = tmp_tokens -> next -> next;
			}
			else if (tmp_tokens->next->type == DOUBLE_QUOTE)
			{
				output = fd_opener(tmp_tokens->next->content, 1);
				tmp_tokens = tmp_tokens -> next;
			}
			else if (tmp_tokens->next->type == SPACE && tmp_tokens->next->next->type == DOUBLE_QUOTE)
			{
				output = fd_opener(tmp_tokens->next->next->content, 1);
				tmp_tokens = tmp_tokens -> next -> next;
			}
